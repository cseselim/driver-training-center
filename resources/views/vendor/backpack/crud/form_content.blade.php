...
@else
    <div class="{{ isset($formInsideCard) && $formInsideCard ? 'card' : (!isset($formInsideCard) ? 'card' : '') }}">
        <div
            class="{{ isset($formInsideCard) && $formInsideCard ? 'card-body' : (!isset($formInsideCard) ? 'card-body' : '') }} row">
                ...
                @if ($focusField)
                    ...
                    if (focusFieldTab !== 'null') {
                        try {
                            ...
                            const formIdInput = currentFormEl ? currentFormEl.querySelector(
                                'input[name="_form_id"]') : null;
                            const theFormId = formIdInput ? formIdInput.value : ('{{ $formId ?? 'crudForm' }}');
                            const selector =
                                `#form_tabs[data-form-id="${theFormId}"] a[tab_name="${focusFieldTab}"]`;
                            $(selector).tab('show');
                        } catch (e) {
                ...
                if (submittedFormId && submittedFormId === currentFormId) {
                            ...
                            $.each(messages, function(key, msg) {
                                ...
                                @if ($crud->tabsEnabled())
                                    var tab_id = $(container).closest('[role="tabpanel"]')
                                        .attr('id');
                                    try {
                                        $('#form_tabs[data-form-id="' + (
                                                    typeof currentFormId !== 'undefined' ?
                                                    currentFormId :
                                                    '{{ $formId ?? 'crudForm' }}') +
                                                '"] [aria-controls="' + tab_id + '"]')
                                            .addClass('text-danger');
                                    } catch (e) {
                                        $("#form_tabs [aria-controls=" + tab_id + "]")
                                            .addClass('text-danger');
                                    }
                                @endif
                            });
                    ...
                    if (firstErrorField !== null) {
                        @if ($crud->tabsEnabled())
                            ...
                            if (firstErrorTab) {
                                try {
                                    var selector = '#form_tabs[data-form-id="' + (typeof currentFormId !==
                                            'undefined' ? currentFormId : '{{ $formId ?? 'crudForm' }}') +
                                        '"] .nav a[href="#' + firstErrorTab + '"]';
                                    $(selector).tab('show');
                                } catch (e) {
